...
class Create_Conditional_Fields {
	...
	public function add_assets_files() {
		...
		wp_localize_script('cfef_logic', 'my_script_vars', array(
	...
	public function append_conditional_fields_controler( $widget ) {
		...
		if ( is_wp_error( $control_data ) ) {
			...
			$field_controls = array(
				'form_fields_conditions_tab' =>
					array(
						...
						'condition'    => array(
							'field_type' => array( 'text', 'email', 'textarea', 'number', 'select', 'radio', 'checkbox', 'tel', 'url', 'date', 'time', 'html' ,'upload', 'recaptcha' , 'recaptcha_v3' ,  'password' , 'acceptance'),
						),
					),
				'cfef_logic'                 => array(
					'name'         => 'cfef_logic',
			...
			$field_controls['cfef_pro_link_button'] = array(
			...
			if ( ! get_option( 'cfef_elementor_notice_dismiss' ) ) {
				...
				$html        .=	'<div id="cfef_elementor_review_dismiss" data-url="' . esc_url( $url ) . '" data-nonce="' . esc_attr( $review_nonce ) . '">Close Notice X</div>
								<div class="cfef_elementor_review_msg">'.__('Hope this addon solved your problem!','cfef')  .'<br><a href="https://wordpress.org/support/plugin/conditional-fields-for-elementor-form/reviews/#new-post/?utm_source=cfef_plugin&utm_medium=inside&utm_campaign=get-pro&utm_content=review-notice" target="_blank"">Share the love with a ⭐⭐⭐⭐⭐ rating.</a><br><br></div>
	...
	public function cfef_check_field_logic( $compare_field_value, $condition_operation, $compare_value ) {
		...
		switch ( $condition_operation ) {
			case '==':
				return $compare_field_value === $compare_value && '' !== $compare_field_value;
			case '!=':
				return $compare_field_value !== $compare_value && '' !== $compare_field_value;
			case '>':
				return $compare_field_value > $compare_value && '' !== $compare_field_value;
	...
	public function all_field_conditions( $instance ) {
		...
		endforeach;
		// Encode and escape the data before outputting.
